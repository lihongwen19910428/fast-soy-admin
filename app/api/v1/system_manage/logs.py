...
@router.post("/logs/search", summary="搜索日志列表")
async def _(log_in: LogSearch):
    log_type = log_in.log_type
    if log_type is None:
        log_type = LogType.ApiLog

    current = log_in.current or 1
    size = log_in.size or 10
    ...
    if "R_ADMIN" in user_role_codes and log_type not in [LogType.ApiLog, LogType.UserLog]:  # 管理员只能查看API日志和用户日志
        return Fail(msg="Permission Denied")
    elif "R_SUPER" not in user_role_codes and "R_ADMIN" not in user_role_codes and log_type != LogType.ApiLog:  # 非超级管理员和管理员只能查看API日志
        return Fail(msg="Permission Denied")

    total, log_objs = await log_controller.list(page=current, page_size=size, search=q, order=["-id"])
    records = []
    for obj in log_objs:
        await obj.fetch_related("api_log", "by_user")
        api_log: APILog = obj.api_log
        by_user_obj: User = obj.by_user
        record = await obj.to_dict(exclude_fields=["by_user_id", "api_log_id"])
        if log_type == LogType.ApiLog:
        ...
        elif log_type == LogType.SystemLog:
            record["logUser"] = "System"
        else:
            if by_user_obj:
                record["byUser"] = str(by_user_obj.id)
                record["byUserInfo"] = await by_user_obj.to_dict(include_fields=["id", "nick_name"])
            else:
                record["byUser"] = None
...
@router.get("/logs/{log_id}", summary="查看日志详情")
