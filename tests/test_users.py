...
class TestUserList:
    async def test_get_user_list(self, auth_client: AsyncClient):
        resp = await auth_client.post("/api/v1/system-manage/users/all/", json={
            "current": 1,
            "size": 10,
        })
...
class TestUserCRUD:
    async def test_create_user(self, auth_client: AsyncClient):
        resp = await auth_client.post("/api/v1/system-manage/users", json={
    ...
    async def test_update_user(self, auth_client: AsyncClient):
        ...
        create_resp = await auth_client.post("/api/v1/system-manage/users", json={
            "userName": "update_me",
            "password": "test123456",
        ...
        resp = await auth_client.patch(f"/api/v1/system-manage/users/{user_id}", json={
